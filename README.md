...
Скрипт используя утилиту zabbix_get выполняет через zabbix агента, установленного на удалённом
 хосте, утилиты:
    <li> 'netstat.exe -bon' -  информация о подключениях окон к серверам ECS;
    <li> 'tasklist.exe /V /FI "IMAGENAME eq  SdrOpStationUI30.exe"  /FO LIST' - информация
          о колличестве окон и их владельах (пользователях).

Вывод парсится скриптом и предоставляется в JSON, на основании в Zabbix формируются
items и triggers.
##Installation
